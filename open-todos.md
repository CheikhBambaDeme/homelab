...
Since 2026-09-14 there is a **second** unbacked Postgres database: GymLog's training history. Lower stakes than a shop's books, but it is still months of data that exists only here, and the phone's IndexedDB holds only the session in progress — it is an outbox, not a replica, so it would not restore anything.
...
docker compose -f ~/docker-apps/gymlog/docker-compose.yml exec -T db \
  pg_dump -U gymlog gymlog | gzip > ~/backups/gymlog-$(date +%F).sql.gz
...
## Both web apps require Tailscale on every device that uses them
...
GymLog is on the same footing, at `gymlog.tail9991b1.ts.net`: Cheikh's phone needs Tailscale to log a set — though the app is built to work entirely offline and sync later, so an unreachable server means a delayed sync rather than a lost session.
