...
GymLog is on the same footing, at `:8443`: Cheikh's phone needs Tailscale to log a set — though the app is built to work entirely offline and sync later, so an unreachable server means a delayed sync rather than a lost session.
